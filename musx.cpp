...
//! -------------------------------------------------------------------------------------------------------
    static Audio_t LoadSoundFromFile(const char* _path)
    {
        ...
        if (!sndfile)
        {
            sprintf_s(Err_Throw, SND_ERRMSG "Could not open audio in %s\n", _path);
            throw Err_Throw;
        }
        if (sfinfo.frames < 1 || sfinfo.frames >(sf_count_t)(INT32_MAX / sizeof(short)) / sfinfo.channels)
        {
            sprintf_s(Err_Throw, SND_ERRMSG "Bad sample count in %s (%" PRId64 ")\n", _path, sfinfo.frames);
            sf_close(sndfile);
            throw Err_Throw;
        }
        ...
        if (num_frames < 1)
        {
            free(membuf);
            sf_close(sndfile);
            sprintf_s(Err_Throw, SND_ERRMSG "Failed to read samples in %s (%" PRId64 ")\n", _path, num_frames);
            throw Err_Throw;
        }
        ...
        if (err != AL_NO_ERROR)
        {
            sprintf_s(Err_Throw, AL_ERRMSG "%s\n", alGetString(err));
    ...
    SoundPlayer::SoundPlayer()
    {
        ...
        alSourcef(source, AL_PITCH, 0.0f);
...
//!--------------------------------------------------------------------------------------
//!--------------------------------------------------------------------------------------
	Engine* Engine::init(const char* _DeviceName, const int* _AttrList)
	{
		static Engine* ret;

		try
		{
			ret = new Engine(_DeviceName, _AttrList);
		}
		catch (Engine* ptr)
		{
			return ptr;
